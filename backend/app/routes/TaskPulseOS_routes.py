...
from backend.app.TaskPulseOS.Commands.SendAlertCommand import SendAlertCommand
...
from backend.app.TaskPulseOS.QuerryHandlers.AlertQueryHandler import alert_query_handler
...
from backend.app.TaskPulseOS.QuerryHandlers.stepRunIDHandler import stepRunIDHandler
...
def pulse_hook_heartbeat():
    ...
    tables interacting: Heartbeat, run_steps, alert
    """
    #rate limitting
    ...
    if not idem_key:
        ...
        if status:
            try:
                ...
                response = {"Error in read_idempotency" : "Error occured"}
            return response  
    #get the latest heartbeat
    latest_db = SelectLatestHB()
    if latest_db["Status"] == "Error":
        return jsonify({"error in SelectLatestHB out": latest_db["Message"]}),500
    #store the heartbeat
    ...
    if result["Status"] == "Error":
        return jsonify({"error SaveHeartbeatCmdHnd out": result["Message"]}),500
    ...
    if heatlthStatus["Status"] == "Healthy":
        ...
        if statusUnhealthySteps["Status"] == "Error":
            return jsonify({"error in HandleUnhealthySteps out ": statusUnhealthySteps["Message"]}),500
        #send alert
        ...
        alertHandlerObj = CreateAlertHandler()
        alertStat = alertHandlerObj.handle(alertCMD)
        if alertStat["Status"] == "Error":
            return jsonify({"error in CreateAlertHandler out": alertStat["Message"]}),500
        return jsonify({"Status": "Warning", "Message": "Heartbeat saved but worker is unhealthy"}),200
    else:
        return jsonify({"error in CheckWorkerHealth out": heatlthStatus["Message"]}),500         
...
def get_run_steps(run_id):
    if run_id is None:
        return jsonify({"Status": "Error","Message":"Run ID is null"})
    query = stepRunIDQuery(runID=run_id)
    result = stepRunIDHandler(command=query)
    if result is None:
        return jsonify({"Status":"Error","Message":"Result is none"}), 200
    else:
        return result
...
def get_alerts():
    ...
    #get the alerts by using above info
    try:
        query = AlertQuery(tenant=tenant,severity=severity)
        result = alert_query_handler(query)
    except Exception as e:
        return jsonify({"error": str(e)}), 500
